...
describe IpBanner do
  ...
  it "can read config file and output error if not found" do
  ...
  it "detects forbidden methods" do
    ...
    ].map{|elt| IpBanner::Request.new(elt, IpBanner::LogNginx.new)}
    ...
    sample_forbidden_methods.each do |request|
      banner.forbidden_method?(request.method, ["GET", "POST"]).should be_true
    end

    sample_allowed_methods.each do |request|
      banner.forbidden_method?(request.method, ["GET", "POST"]).should be_false
    end
  end
  ...
  it "detects forbidden paths" do
    ...
    sample_forbidden_paths.each do |request|
      banner.forbidden_path?(request.path, ["/", "/solr/admin/info/system?wt=json"]).should be_true
    end

    sample_allowed_paths.each do |request|
      banner.forbidden_path?(request.path, ["/", "/solr/admin/info/system?wt=json"]).should be_false
    end
